  ...
  async parseCanister(): Promise<ICRC1[]> {
    ...
    while (isRequestNeeded) {
      const response = await chai
        .request(`https://3r4gx-wqaaa-aaaaq-aaaia-cai.icp0.io/v1/sns/list/page/${page}/slow.json`)
        .get('');
    ...
    const canisterPromises = data.map((l) => {
      ...
      return chai
        .request(`https://sns-api.internetcomputer.org`)
        .get(`/api/v1/snses/${l.canister_ids.root_canister_id}`)
        .then((a) => {
    ...
    const canisters = (await Promise.all(
      canisterPromises.filter((c) => c !== undefined)
    )) as ICRC1[];
