...
describe('Swap Trs Storage test', () => {
  ...
  it('Store/Get transactions', async function () {
    ...
    let trss: SwapTransaction[] = await storageActor.get_transactions(
    ...
    execute(
      `dfx deploy swap_trs_storage  --argument '(opt record { im_canister = principal "${dfx.im.id}" })' --upgrade-unchanged`
    );
