...
describe('Delegation Factory test', () => {
  ...
  it('Get delegation', async function () {
    ...
    const chain = await dfActor
      .get_delegation(response.anchor, 'nfid.one', pk, prepareDelegationResponse[1], targets)
            ...
            [
              {
                ...
                signature: new Uint8Array(r.signed_delegation.signature).buffer as Signature,
              },
            ],
    ...
    const delegationIdentity = DelegationIdentity.fromDelegation(sessionPair, chain);
  ...
  it('Get delegation - Unauthorized', async function () {
    try {
      let resp = await dfActor.prepare_delegation(100000002n, 'nfid.one', pk, [], targets);
      await dfActor.get_delegation(100000002n, 'nfid.one', pk, resp[1], targets);
    } catch (e) {
  ...
  it('Add operator and clean the memory', async function () {
    ...
    let resp = await dfActor.prepare_delegation(100000000n, 'nfid.one', pk, [], targets);
    let delegation = await dfActor.get_delegation(100000000n, 'nfid.one', pk, resp[1], targets);

    expect(delegation.signed_delegation).not.undefined;

    await dfActor.clean_memory();

    let response = await dfActor.get_delegation(100000000n, 'nfid.one', pk, resp[1], targets);
