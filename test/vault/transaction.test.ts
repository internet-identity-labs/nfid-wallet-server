...
describe.skip('Transaction', () => {
  ...
  before(async () => {
    ...
    wallet3 = (await dfx.vault.actor_member_2.register_wallet({
      name: ['Wallet2'],
      vault_id: 3n,
    })) as Wallet;
  });
  ...
  it('Transaction register required 1 approves', async function () {
    ...
    let transactionsMember =
      (await dfx.vault.actor_member_1.get_transactions()) as Array<Transaction>;
  ...
  it('Transaction insufficient funds', async function () {
    ...
    expect(memo).contains(
      'ledger transfer error: InsufficientFunds { balance: Tokens { e8s: 300000 } }'
    );
  ...
  it('Get backup', async function () {
    ...
    let vaultsString = (await dfx.vault.admin_actor.get_all_json(0, 10, {
      Vaults: null,
    })) as string;
    let vaults = JSON.parse(vaultsString) as [Vault];
    expect(vaults.length).eq(3);
    expect(cVaults).eq(3n);
    let walletString = (await dfx.vault.admin_actor.get_all_json(0, 10, {
      Wallets: null,
    })) as string;
    ...
    let transactionsString = (await dfx.vault.admin_actor.get_all_json(0, 10, {
      Transactions: null,
    })) as string;
    ...
    let policyString = (await dfx.vault.admin_actor.get_all_json(0, 10, {
      Policies: null,
    })) as string;
  ...
  function verifyTransaction(actual: Transaction, expected: Transaction) {
    ...
    expect(Object.keys(actual.currency)[0]).eq(Object.keys(expected.currency)[0]);
