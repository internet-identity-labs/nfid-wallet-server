...
describe('Policy', () => {
  ...
  before(async () => {
    ...
    wallet1 = (await dfx.vault.admin_actor.register_wallet({
      name: ['Wallet1'],
      vault_id: 2n,
    })) as Wallet;
    wallet2 = (await dfx.vault.admin_actor.register_wallet({
      name: ['Wallet2'],
      vault_id: 1n,
    })) as Wallet;
  });
  ...
  it('define correct policy both less than', async function () {
    ...
    let tokens = 100n;
    let registerRequest: TransactionRegisterRequest = {
      address: to,
      amount: tokens,
  ...
  it('expect: unable to find policy', async function () {
    ...
    let wallet3 = (await dfx.vault.admin_actor.register_wallet({
      name: ['Wallet2'],
      vault_id: vault.id,
    })) as Wallet;
...
function verifyPolicy(actual: Policy, expected: Policy) {
  ...
  expect(actual.policy_type.threshold_policy.member_threshold.length).eq(
    expected.policy_type.threshold_policy.member_threshold.length
  );
  expect(actual.policy_type.threshold_policy.amount_threshold).eq(
    expected.policy_type.threshold_policy.amount_threshold
  );
  expect(actual.policy_type.threshold_policy.wallets.length).eq(
    expected.policy_type.threshold_policy.wallets.length
  );
  expect(Object.keys(actual.policy_type.threshold_policy.currency)[0]).eq(
    Object.keys(expected.policy_type.threshold_policy.currency)[0]
  );
  if (
    actual.policy_type.threshold_policy.wallets.length > 0 &&
  ...
  if (actual.policy_type.threshold_policy.member_threshold.length > 0) {
    expect(actual.policy_type.threshold_policy.member_threshold[0]).eq(
      expected.policy_type.threshold_policy.member_threshold[0]
    );
  }
}
