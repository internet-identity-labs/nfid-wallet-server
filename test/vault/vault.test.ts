...
describe('Vault', () => {
  ...
  it('add_member', async function () {
    ...
    vault = ((await dfx.vault.admin_actor.get_vaults()) as [Vault]).find((l) => l.id === 1n);
    verifyVault(vault, expected);

    let vaultMember = ((await dfx.vault.actor_member_1.get_vaults()) as [Vault]).find(
      (l) => l.id === 1n
    );
  ...
  it('update vault/member', async function () {
    let vault = ((await dfx.vault.actor_member_1.get_vaults()) as [Vault]).find((l) => l.id === 1n);
...
function verifyVault(actualVault: Vault, expectedVault: Vault) {
  ...
  for (const actMember of actualVault.members) {
    verifyMember(
      actMember,
      expectedVault.members.find((l) => l.user_uuid === actMember.user_uuid)
    );
  }
}
