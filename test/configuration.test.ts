...
describe('Configuration', () => {
  ...
  it('should respond with correct configuration', async function () {
    const result: ConfigurationResponse =
      (await dfx.im.actor.get_config()) as ConfigurationResponse;
    ...
    expect(result.lambda[0]?.toText()).to.be.equal(
      '3ekng-5nqql-esu4u-64sla-pcm5o-hjatn-hwjo7-vk7ya-ianug-zqqyy-iae'
    );
  ...
  it('should update env field in the configuration', async function () {
    ...
    const configureResponse: ConfigurationResponse =
      (await dfx.im.actor.get_config()) as ConfigurationResponse;
    ...
    expect(configureResponse.ii_canister_id[0]?.toText()).to.be.equal(
    ...
    expect(configureResponse.lambda[0]?.toText()).to.be.equal(
      'ritih-icnvs-i7b67-sc2vs-nwo2e-bvpe5-viznv-uqluj-xzcvs-6iqsp-fqe'
    );
  ...
  it('should block/unblock account creation', async function () {
    ...
    let email_response = (await dfx.im.actor.add_email_and_principal_for_create_account_validation(
      'test@test.test',
      principal,
      25n
    )) as BoolHttpResponse;
    ...
    try {
      ...
      expect(e.message).to.contain(
        'Account creation is paused due to high demand. Please try again later.'
      );
    }
