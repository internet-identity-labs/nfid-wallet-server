...
export const deploy = async ({
  ...
  while (++i <= 5) {
    ...
    if (apps.includes(App.InternetIdentityTest)) {
      ...
      dfx.iit.actor = await getTypedActor<InternetIdentityTest>(
        dfx.iit.id,
        dfx.user.identity,
        iitIdl
      );
    ...
    if (apps.includes(App.ICRC1Oracle)) {
      ...
      dfx.icrc1_oracle.actor = await getActor(
        dfx.icrc1_oracle.id,
        dfx.user.identity,
        icrcOracle1Idl
      );
    ...
    if (apps.includes(App.DelegationFactory)) {
      execute(
        `dfx deploy delegation_factory --mode reinstall -y --argument '(opt record { im_canister = principal "${dfx.im.id}" })'`
      );

      dfx.delegation_factory.id = DFX.GET_CANISTER_ID('delegation_factory');
      console.log('>> ', dfx.delegation_factory.id);

      dfx.delegation_factory.actor = await getActor(
        dfx.delegation_factory.id,
        dfx.user.identity,
        delegationFactoryIDL
      );
      return dfx;
    }
    if (apps.includes(App.NFIDStorage)) {
      execute(
        `dfx deploy nfid_storage --mode reinstall -y --argument '(opt record { im_canister = principal "${dfx.im.id}" })'`
      );

      dfx.nfid_storage.id = DFX.GET_CANISTER_ID('nfid_storage');
      console.log('>> ', dfx.nfid_storage.id);

      dfx.nfid_storage.actor = await getActor(
        dfx.nfid_storage.id,
        dfx.user.identity,
        nfidStorageIDL
      );
    ...
    if (apps.includes(App.SwapTrsStorage)) {
      execute(
        `dfx deploy swap_trs_storage --mode reinstall -y --argument '(opt record { im_canister = principal "${dfx.im.id}" })'`
      );

      dfx.swap_trs_storage.id = DFX.GET_CANISTER_ID('swap_trs_storage');
      console.log('>> ', dfx.swap_trs_storage.id);

      dfx.swap_trs_storage.actor = await getActor(
        dfx.swap_trs_storage.id,
        dfx.user.identity,
        swapTrsStorageIDL
      );
      return dfx;
