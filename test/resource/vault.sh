...
#VAULT_ACCOUNT_ID_BYTES="$(python3 -c 'print("vec{" + ";".join([str(b) for b in bytes.fromhex'$$VAULT_ACCOUNT_SUB_BYTES']) + "}")')"
#dfx canister call ledger transfer "(record { to=$VAULT_ACCOUNT_SUB_BYTES; amount=record { e8s=100_000_000 }; fee=record { e8s=10_000 }; memo=0:nat64; }, )"
echo "WALLET_1"

#dfx canister call ledger transfer "(record { to=${VAULT_ACCOUNT_ID_BYTES}; amount=record { e8s=100_000_000 }; fee=record { e8s=10_000 }; memo=0:nat64; }, )"


#echo $VAULT_ACCOUNT_SUB_ID
#
#echo $VAULT_ACCOUNT_SUB_BYTES
#echo $VAULT_ACCOUNT_ID_BYTES
#
