...
describe('Account', () => {
  describe('CLI tests', () => {
    ...
    it('should throw error due to existing anchor.', async function () {
      ...
      expect(DFX.CREATE_ACCOUNT_FULL()).eq(
        Expected.ERROR('Impossible to link this II anchor, please try another one.', '404')
      );
    });
    ...
    it('should remove account and create new one.', async function () {
      ...
      expect(DFX.REMOVE_ACCOUNT('identity_manager')).eq(
        Expected.ERROR('Unable to remove Account', '404')
      );
    ...
    it('should remove account and create new one with email and receive an error.', async function () {
      expect(DFX.REMOVE_ACCOUNT('identity_manager')).eq(Expected.BOOL('true', '200'));
      expect(DFX.CREATE_ACCOUNT_WITH_EMAIL('12345', 'opt "test@test.test"')).contains(
        'Email and principal are not valid.'
      );
    });
  });
  ...
  describe('Agent tests', () => {
    ...
    it('should return an error by adding new email with bigger timestamp due to self cleaning of the service', async function () {
      let response1 = (await dfx.im.actor.add_email_and_principal_for_create_account_validation(
        'testdefault@test.test',
        dfx.user.principal,
        900000n
      )) as BoolHttpResponse;
      ...
      let response2 = (await dfx.im.actor.add_email_and_principal_for_create_account_validation(
        '1@test.test',
        dfx.user.principal,
        BigInt(900000 * 2 + 1)
      )) as BoolHttpResponse;
    ...
    it('should enable 2fa', async function () {
      ...
      let root = (await actorDevice.get_root_by_principal(
        identityDevice.getPrincipal().toText()
      )) as string;
    ...
    it('should sync recovery phrase from Internat Identity', async function () {
      ...
      const actorII = await getTypedActor<InternetIdentityTest>(
        dfx.iit.id,
        rootAccessPointIdentity,
        iitIdl
      );
      ...
      const actor = await getTypedActor<IdentityManagerType>(
        dfx.im.id,
        rootAccessPointIdentity,
        imIdl
      );
      ...
      const recoveryPhraseOldPubkey = Array.from(
        new Uint8Array(recoveryPhraseOldIdentityIdentity.getPublicKey().toDer())
      );
      ...
      const recoveryPhraseActor = await getTypedActor<IdentityManagerType>(
        dfx.im.id,
        recoveryPhraseIdentity,
        imIdl
      );
      ...
      const recoveryPhraseResponse =
        await recoveryPhraseActor.sync_recovery_phrase_from_internet_identity(
          accountResponse.data[0].anchor
        );
      ...
      const recoveryDevice = getAccountResponseFound.data[0].access_points.find(
        (x) => 'Recovery' in x.device_type
      );
    ...
    it('Should get captcha and create account with it', async function () {
      ...
      let accThroughCaptcha: HTTPAccountResponse = (await actor.create_account(
        accountRequest
      )) as HTTPAccountResponse;
